...
const Diseases = () => {
	const { items, error, loading, handleDelete } = useDiseases();
	...
	const data = () => {
		...
		items.forEach((item) => {
			formated.push({
				...
				options: (
					<Fragment>
						<Link to={`diseases/${item.id}/edit`} className="btn btn-success btn-sm">
							Editar
						</Link>
	...
	return (
		<MDBContainer>
			<Link to={`diseases/new`} className="btn btn-primary btn-sm">
				Crear
			</Link>
			<MDBDataTable
				striped
