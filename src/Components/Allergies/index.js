...
const Allergies = () => {
	const { items, error, loading, handleDelete } = useAllergies();
	...
	const data = () => {
		...
		items.forEach((item) => {
			formated.push({
				...
				options: (
					<Fragment>
						<Link to={`allergies/${item.id}/edit`} className="btn btn-success btn-sm">
							Editar
						</Link>
	...
	return (
		<MDBContainer>
			<Link to={`allergies/new`} className="btn btn-primary btn-sm">
				Crear
			</Link>
			<MDBDataTable
				striped
