...
import useReligions from './useReligions';
...
const Religions = () => {
	const { items, error, loading, handleDelete } = useReligions();
	...
	const data = () => {
		...
		items.forEach((item) => {
			formated.push({
				...
				options: (
					<Fragment>
						<Link to={`religions/${item.id}/edit`} className="btn btn-success btn-sm">
							Editar
						</Link>
	...
	return (
		<MDBContainer>
			<Link to={`religions/new`} className="btn btn-primary btn-sm">
				Crear
			</Link>
			<MDBDataTable
				striped
