...
const MedicalAnalysis = () => {
	const { items, error, loading, handleDelete } = useMedicalAnalysis();
	...
	const data = () => {
		...
		items.forEach((item) => {
			formated.push({
				...
				options: (
					<Fragment>
						<Link to={`medicalanalysis/${item.id}/edit`} className="btn btn-success btn-sm">
							Editar
						</Link>
	...
	return (
		<MDBContainer>
			<Link to={`medicalanalysis/new`} className="btn btn-primary btn-sm">
				Crear
			</Link>
			<MDBDataTable
				striped
